...
async function all_process(orgId){
    const result =  await global.pool.query("SELECT processflow_metadata ->0->0->> 'DefinitionId' as DefinitionId, processflow_metadata ->0->0->> 'FullName' as FullName FROM orginformation o where orgid=$1 order by createddate  desc fetch first row only", [orgId]);
